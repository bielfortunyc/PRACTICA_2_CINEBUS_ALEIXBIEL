...
class Film:
    ...
    def __init__(
        ...
        """Inicialitza la classe film a partir d'un
        diccionari amb les claus title, genre, directors
        i actors."""
...
class Cinema:
    ...
    def __init__(self, name: str, address: str,
                 coordenades: tuple[float, float]) -> None:
        """Inicialitza la classe Cinema donat el nom, adreça
        i coordenades, que es treuen d'una llista fixa."""
        self.name = name
...
class Projection:
    ...
    def __init__(self, film: Film, cinema: Cinema,
                 time: tuple[int, int],
                 language: str = 'Castellà') -> None:
        """Inicialitza la classe Projection donada una
        pel·licula, cinema i hora."""
    ...
    def time(self) -> tuple[int, int]:
        """Retorna l'hora de començament de la projeccio.
        Hora, minut"""
        return self._time
...
class Billboard:
    ...
    def __init__(
        ...
        """Inicialitza la classe Billboard donades una
        llista de pel·licules, cinemes i projeccions."""
        self._films, self._cinemes = films, cinemes
    ...
    def troba_mot(self, m: str) -> list[Projection]:
        """Retorna una llista de projeccions que tenen
        el mot m al titol de la pel·licula i nom del
        cinema."""
        matching_mots = [
            element for element in self.projections()
            if m in element.film().title or m in
    ...
    def troba_actor(self, m: str) -> list[Projection]:
        """Retorna una llista de projeccions les
        pel·licules de les qual tenen l'actor m.
        Prec: s'ha d'escriure el nom sencer."""
        matching_mots = [
            element for element in self.projections()
            if m in element.film().actors]
        return matching_mots

    def troba_director(self, m: str) -> list[Projection]:
        """Retorna una llista de projeccions les pel·licules
        de les qual tenen el director m.
        Prec: s'ha d'escriure el nom sencer."""
        matching_mots = [
            element for element in self.projections()
            if m in element.film().director]
        return matching_mots

    def troba_genere(self, m: str) -> list[Projection]:
        """Retorna una llista de projeccions les pel·licules
        de les qual són del genere m.
        Prec: s'ha d'escriure correctament."""
        matching_mots = [
            element for element in self.projections()
            if m in element.film().genre]
        return matching_mots

    def cerca_cinema(self) -> list[Projection]:
        """Donada la cartellera es retorna la llista
        de projeccions que es duen a terme al cinema
        que es demana."""
        console = Console()
    ...
    def cerca_horari(self) -> list[Projection]:
        """Donada la cartellera es retorna la llista
        de projeccions a l'hora que es demana."""
        console = Console()
        console.print(
            "A quina Hora vols anar al cine? Escriu",
            "l'hora en punt. ", end='', style="light_pink3")
        try:
            pel = int(input())
        except Exception:
            return list()
    ...
    def cerca_pelicula(self) -> list[Projection]:
        """Donada la cartellera es retorna la llista
        de projeccions de la pel·licula que es demana."""
        console = Console()
        ...
        sessions: list[Projection] = list()
        sessions = [x for x in self.projections()
                    if pel in x.film().title]

        return sessions


def read() -> Billboard:
    """Funció principal del programa billboard que
    llegeix la cartellera del cinemes de Barcelona
    i la retorna en la classe Billboard."""
...
def cinemes() -> dict[str, Cinema]:
    """Llista que recull les dades dels cinemes de
    Barcelona i les retorna com a diccionari amb el
    nom del cinema i el tipus Cinema."""
    cinemes: dict[str, Cinema] = dict()
    cinemes['Arenas Multicines 3D'] = Cinema(
        'Arenas Multicines 3D',
        'Gran Via de les CortsCatalanes, 385, 08015 Barcelona',
        (2.149546, 41.376047))
    ...
    cinemes['Bosque Multicines'] = Cinema(
        ...
        (2.1516103914622153, 41.401519050000005))
    cinemes['Cinema Comedia'] = Cinema(
        'Cinema Comedia',
        'Passeig de Gracia, 13, 08007 Barcelona',
        (2.167663, 41.389718))
    cinemes['Cinemes Girona'] = Cinema(
        'Cinemes Girona',
        'Carrer de Girona 173-175, 08025 Barcelona',
        (2.1642872, 41.399941))
    cinemes['Cines Verdi Barcelona'] = Cinema(
        'Cines Verdi Barcelona',
        'Calle Verdi, 32, 08012 Barcelona',
        (2.156800, 41.403920))
    cinemes['Cinesa Diagonal 3D'] = Cinema(
        'Cinesa Diagonal 3D',
        'Santa Fe de Nou Mèxic s/n, 08017 Barcelona',
        (2.136119, 41.393731))
    cinemes['Cinesa Diagonal Mar 18'] = Cinema(
        'Cinesa Diagonal Mar 18',
        'Avinguda Diagonal, 3, 08019 Barcelona',
        (2.2172596, 41.4101085))
    cinemes['Cinesa La Maquinista 3D'] = Cinema(
        'Cinesa La Maquinista 3D',
        'Passeig Potosí 2, 08030 Barcelona', (2.198340, 41.439119))
    ...
    cinemes['Yelmo Cines Icaria 3D'] = Cinema(
        ...
        'Calle Salvador Espriu, 61, 08005 Barcelona',
        (2.197470, 41.390225))
    cinemes['Boliche Cinemes'] = Cinema(
        'Boliche Cinemes',
        'Avinguda Diagonal, 508, 08006 Barcelona',
        (2.1536309, 41.3952918))
    cinemes['Zumzeig Cinema'] = Cinema(
        'Zumzeig Cinema',
        'Carrer Béjar, 53, 08014 Barcelona',
        (2.1450266, 41.3773203))
    cinemes['Balmes Multicines'] = Cinema(
        'Balmes Multicines',
        'Calle Balmes, 422-424, 08022 Barcelona',
        (2.139344, 41.407308))
    cinemes['Maldá Arts Forum'] = Cinema(
        'Maldà Arts Forum',
        'Carrer del Pi, 5, 08002 Barcelona',
        (2.1739003, 41.3832363))
    # Només hem afegit els cinemes dins el municipi
    # de Barcelona, una part de la cartellera de Sensacine
    # s'ha tret.
    return cinemes


def llista_cinema(elements: Any) -> list[Cinema]:
    """Donat un fitxer html que conté els elements
    dels cinemes en retorna una llista d'ells."""
...
def llista_films(elements: Any) -> list[Film]:
    """Donat un fitxer html que conté els elements
    de les pel·licules en retorna una llista d'ells."""
    films: dict[str, Film] = dict()
    for element in elements:
        content = str(element)
        match = re.search(r'data-movie=\'(.*?)\'',
                          content)
        if match:
...
def llista_projeccions(elements: Any) -> list[Projection]:
    """Donat un fitxer html que conté els elements de les
    projeccions en retorna una llista d'ells."""
    projections: list[Projection] = list()
    repetits: dict[str, set[str]] = dict()
    for element in elements:

        content = str(element)
        # Utilitza expressió regular per trobar els valors
        # de l'atribut data-times
        match2 = re.search(r'data-theater=\'(.*?)\'',
                           content)
        if match2:
        ...
        data_times = [em.get('data-times')
                      for em in em_elements]
        for dt in data_times:
            # S'agafen totes les projeccions per la pel·lícula
            # i cinema actuals. S'eviten repeticions i agafar la
            # informació dels dies següents.
